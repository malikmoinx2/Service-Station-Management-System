...
const convertTo24Hour = (timeStr) => {
  if (!timeStr) return "";
  const normalized = timeStr.toUpperCase().replace(/\s/g, ''); // Remove all spaces
...
const checkIsVip = (start, end) => {
  if (!start || !end) return false;
  const s24 = start.includes('AM') || start.includes('PM') ? convertTo24Hour(start) : start;
  const e24 = end.includes('AM') || end.includes('PM') ? convertTo24Hour(end) : end;
  const [sh, sm] = s24.split(':').map(Number);
  const [eh, em] = e24.split(':').map(Number);
  const sMins = sh * 60 + sm;
  const eMins = eh * 60 + (em || 0);
  return sMins < 840 && eMins > 780;
};

// ─────────────────────────────────────────────────────────────────────────────
// COMPONENT
// ─────────────────────────────────────────────────────────────────────────────
const viewnearbystations = ({ navigation }) => {
  const {
  ...
  const [isAllSlotsLoading, setIsAllSlotsLoading] = useState(false);
  ...
  // ────────────────────────────────────────────────────────────────────────
  useEffect(() => {
    console.log("DEBUG: bookingData in viewnearbystations:", bookingData);
    if (bookingData?.date && bookingData?.startTime) fetchInitialStations();
  ...
  const fetchInitialStations = async () => {
    try {
      ...
      const res = await fetch(`${BASE_URL}/Customer/check-station-availability`, {
    ...
    try {
      ...
      const matched = raw
        ...
        .map(s => {
          ...
          // Calculate price for each selected vehicle
          const vehiclePrices = selectedVehicles.map(v => {
            const vType = (v.carType || '').toLowerCase();
            let p = 0;
            if (!isVip) {
              p = vType === 'small' ? fNS : fNL;
            } else {
              p = vType === 'small' ? fVS : fVL;
            }
            return { price: p, vehicleName: `${v.carCompany} ${v.carModel}`, type: vType };
          });

          // Fallback if selectedVehicles is empty
          if (vehiclePrices.length === 0) {
            const vType = (bookingData?.carType || '').toLowerCase();
            const p = isVip ? (vType === 'small' ? fVS : fVL) : (vType === 'small' ? fNS : fNL);
            vehiclePrices.push({ price: p, vehicleName: bookingData?.vehicleName, type: vType });
          }
          ...
          return {
            ...
            vehiclePrices,
            price: vehiclePrices.reduce((a, b) => a + b.price, 0),
            hasDiscount,
            discountLabel,
            oNS, oNL, oVS, oVL,
            fNS, fNL, fVS, fVL
  ...
  const enrichList = (list, isAlternative) =>
    list.map(s => ({
      ...
      availableBays: s.availableBays || s.AvailableBays || [],
    }));
  ...
  const fetchAllSlots = async (station, bay) => {
    setIsAllSlotsLoading(true);
    try {
      const res = await fetch(`${BASE_URL}/Customer/get-available-time-slots`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
    ...
    finally { setIsAllSlotsLoading(false); }
  };
  ...
  const proceedWithFreeStation = (item) => {
    ...
    const availableBays = item.availableBays || item.AvailableBays || [];
    const selectedVehicles = bookingData?.selectedVehicles || [];

    // Logic for 2 vehicles:
    // If we have 2+ free bays, we can book parallel.
    // If only 1 free bay, we book sequential.
    let updatedBookingData = {
      ...
      startTime: formatTimeFrom24(bookingData.startTime),
      endTime: formatTimeFrom24(bookingData.endTime),
    };

    if (selectedVehicles.length > 1) {
      if (availableBays.length >= selectedVehicles.length) {
        updatedBookingData.bookingMode = 'parallel';
      } else {
        updatedBookingData.bookingMode = 'sequential';
        const baseDuration = bookingData.duration || 60;
        const totalDuration = baseDuration * selectedVehicles.length;
        const start24 = convertTo24Hour(bookingData.startTime);
        const end24 = shiftTime(start24, totalDuration / 60);
        updatedBookingData.endTime = formatTimeFrom24(end24);
      }
    } else {
      updatedBookingData.availableBayId = availableBays[0]?.bayId || availableBays[0]?.BayId;
    }
    const isActuallyVip = checkIsVip(updatedBookingData.startTime, updatedBookingData.endTime);
    updatedBookingData.isVip = isActuallyVip;

    const finalSvcList = svcList.map(s => {
      const updatedVehiclePrices = (s.vehiclePrices || []).map(vp => {
        let vP = vp.price;
        if (isActuallyVip) vP = vp.type === 'small' ? s.fVS : s.fVL;
        else vP = vp.type === 'small' ? s.fNS : s.fNL;
        return { ...vp, price: vP };
      });
      return {
        ...s,
        price: updatedVehiclePrices.reduce((a, b) => a + b.price, 0) || s.finalPrice,
        vehiclePrices: updatedVehiclePrices
      };
    });

    updatedBookingData.selectedServicesList = finalSvcList;
    updatedBookingData.totalAmount = finalSvcList.reduce((a, s) => a + parseFloat(s.price || 0), 0);

    setBookingData(updatedBookingData);
    setstationid(item.stationId);
    if (availableBays.length > 0) setselectedbayid(availableBays[0]?.bayId || availableBays[0]?.BayId);
    navigation.navigate('addbookingdetails'); // Fixed to match your file name
  };

  const proceedWithSlot = (slot) => {
    const isNewSlotVip = checkIsVip(slot.start, slot.end);
    const carType = (bookingData?.carType || '').toLowerCase();
    const selectedVehicles = bookingData?.selectedVehicles || [];
    const slotBays = slot.bays || slot.Bays || [];
    ...
    let updatedBookingData = {
      ...bookingData,
      startTime: formatTimeFrom24(slot.start),
      endTime: formatTimeFrom24(slot.end),
      ...
      isVip: !!isNewSlotVip,
      availableBays: slotBays,
    };

    if (selectedVehicles.length > 1) {
      if (slotBays.length >= selectedVehicles.length) {
        updatedBookingData.bookingMode = 'parallel';
      } else {
        updatedBookingData.bookingMode = 'sequential';
        const baseDuration = bookingData.duration || 60;
        const start24 = convertTo24Hour(slot.start);
        const end24 = shiftTime(start24, (baseDuration * selectedVehicles.length) / 60);
        updatedBookingData.endTime = formatTimeFrom24(end24);
      }
    } else {
      updatedBookingData.availableBayId = slotBays[0]?.bayId || slotBays[0]?.BayId;
    }

    setBookingData(updatedBookingData);
    setstationid(slotModalStation.stationId);
    if (slotBays.length > 0) setselectedbayid(slotBays[0]?.bayId || slotBays[0]?.BayId);
    closeSlotModal();
    navigation.navigate('addbookingdetails'); // Fixed to match your file name
  };
  ...
  const formatTimeFrom24 = (time24) => {
    ...
    if (time24.includes('AM') || time24.includes('PM')) {
        // Ensure 2-digit padding for SQL server e.g. "1:00 PM" -> "01:00 PM"
      ...
      <View style={styles.card}>
        ...
        <View style={styles.cardTop}>
          <View style={styles.imageColumn}>
            ...
            >
              ...
              <Text style={styles.ratingText}>
                {item.rating > 0 ? item.rating.toFixed(1) : 'New'}({item.totalReviews})
              </Text>
            </TouchableOpacity>
            <Image
              source={{
          ...
          <View style={styles.details}>
            ...
            {/* More Prominent Free Bays & Booking Mode UI */}
            <View style={[styles.bayInfoContainer, { borderLeftColor: isAlternative ? '#FFA000' : '#4CAF50' }]}>
              <View style={styles.bayStatusRow}>
                <Text style={{ fontSize: 16, marginRight: 8 }}>{isAlternative ? '⚠️' : '✅'}</Text>
                <View>
                  <Text style={styles.bayStatusText}>
                    {isAlternative
                      ? `Busy: ${item.busyBays?.length || 0} Bay(s) Occupied`
                      : `Free: ${item.availableBays?.length || 0} Bay(s) Available`
                    }
                  </Text>
                  <Text style={{ fontSize: 10, color: '#666' }}>
                    Booking for {bookingData?.selectedVehicles?.length || 1} Vehicle(s)
                  </Text>
                </View>
              </View>

              {(bookingData?.selectedVehicles?.length > 1) && (
                <View style={[
                  styles.modeBadge,
                  {
                    backgroundColor: (item.availableBays?.length || 0) >= bookingData.selectedVehicles.length ? '#E1F5FE' : '#FFF3E0',
                    borderColor: (item.availableBays?.length || 0) >= bookingData.selectedVehicles.length ? '#01579B' : '#E65100',
                    borderWidth: 1.5
                  }
                ]}>
                  <Text style={[
                    styles.modeBadgeText,
                    { color: (item.availableBays?.length || 0) >= bookingData.selectedVehicles.length ? '#01579B' : '#E65100' }
                  ]}>
                    {(item.availableBays?.length || 0) >= bookingData.selectedVehicles.length
                      ? "⚡ PARALLEL: Dono Saath Mein"
                      : "🕐 SEQUENTIAL: Bari Bari (Line)"}
                  </Text>
                </View>
              )}
            </View>
                ...
                <View key={idx} style={styles.serviceRow}>
                  ...
                  <View style={styles.serviceRight}>
                    ...
                    <View style={{ alignItems: 'flex-end' }}>
                      {s.vehiclePrices?.map((vp, vidx) => (
                        <View key={vidx} style={{ flexDirection: 'row', alignItems: 'center' }}>
                          <Text style={{ fontSize: 10, color: '#666', marginRight: 4 }}>
                            {vp.vehicleName} ({vp.type}):
                          </Text>
                          <Text style={styles.servicePrice}>RS {Math.round(vp.price)}</Text>
                        </View>
                      ))}
                      {s.vehiclePrices?.length > 1 && (
                        <View style={{ borderTopWidth: 1, borderTopColor: '#EEE', marginTop: 2, paddingTop: 2 }}>
                          <Text style={[styles.servicePrice, { color: 'royalblue' }]}>
                            Sub: RS {Math.round(s.price)}
                          </Text>
                        </View>
                      )}
                    </View>
        ...
        <View style={styles.cardBottom}>
          ...
          {!isAlternative ? (
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                setSlotModalStation(item);
                setSlotModalBay(null); // Search for all bays by default
                setSlotModalMode('options');
              }}
            >
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 6 }}>
            ...
            >
              <Text style={[styles.filterChipText, minRating > 0 && styles.filterChipTextActive]}>
                ⭐ {minRating > 0 ? `${minRating}+` : 'Station Rating'} ▾
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
            ...
            >
              <Text style={[styles.filterChipText, minSvcRating > 0 && styles.filterChipTextActive]}>
                🔧 {minSvcRating > 0 ? `${minSvcRating}+` : 'Svc Rating'} ▾
              </Text>
            </TouchableOpacity>
          </ScrollView>
        </View>
          ...
          <View style={styles.modalSheet}>
              ...
              <>
                ...
                <View style={styles.selectedBayBadge}>
                  <Text style={styles.selectedBayBadgeText}>
                    🔧 Bay: {slotModalBay?.bayName || slotModalBay?.BayName || 'All'}
                  </Text>
                </View>
                <Text style={styles.modalSubtitle}>
                  {slotModalBay ? `Bay "${slotModalBay.bayName || slotModalBay.BayName}" is busy` : 'Station is busy'} at your selected time. Choose an option:
                </Text>
                ...
                <TouchableOpacity style={styles.slotOptionCard} onPress={goToShiftMode}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.slotOptionDesc}>
                      Shift your time +/− hours and check availability.
                    </Text>
                  </View>
                  <Text style={styles.slotOptionArrow}>›</Text>
                </TouchableOpacity>
                ...
                <TouchableOpacity style={styles.slotOptionCard} onPress={goToAllSlotsMode}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.slotOptionDesc}>
                      See all free time slots for today.
                    </Text>
                  </View>
                  <Text style={styles.slotOptionArrow}>›</Text>
                </TouchableOpacity>
                ...
                {shiftLoading ? (
                  ...
                  <>
                    <Text style={styles.shiftResultLabel}>
                      ✅ {slotModalBay?.bayName || slotModalBay?.BayName || 'Station'} is free at {formatTimeFrom24(shiftPreview)}:
                    </Text>
                    <ScrollView style={{ maxHeight: 160 }}>
                      {shiftedSlots.map((slot, i) => {
                        const isVip = checkIsVip(slot.start, slot.end);
                        return (
                          <TouchableOpacity key={i} style={[styles.shiftSlotRow, isVip && { borderColor: '#FFD700', borderWidth: 1, backgroundColor: '#FFFDF0' }]} onPress={() => proceedWithSlot(slot)}>
                            <View style={{ flex: 1 }}>
                              <Text style={styles.shiftSlotTime}>
                                {formatTimeFrom24(slot.start)} – {formatTimeFrom24(slot.end)}
                                {isVip && <Text style={{ color: '#B8860B', fontWeight: 'bold', fontSize: 12 }}>  ⭐ VIP</Text>}
                              </Text>
                              <Text style={styles.shiftSlotBay}>
                                🟢 {(slot.bays || slot.Bays || []).length} Bays Free: {(slot.bays || slot.Bays || []).map(b => b.bayName || b.BayName).join(', ')}
                              </Text>
                              {bookingData?.selectedVehicles?.length > 1 && (
                                <Text style={{
                                  fontSize: 10, fontWeight: '700',
                                  color: (slot.bays || slot.Bays || []).length >= bookingData.selectedVehicles.length ? '#1976D2' : '#E65100',
                                  marginTop: 2
                                }}>
                                  {(slot.bays || slot.Bays || []).length >= bookingData.selectedVehicles.length
                                    ? "⚡ Parallel Booking"
                                    : "🕐 Sequential Booking"}
                                </Text>
                              )}
            ...
            {slotModalMode === 'all' && (
              <>
                ...
                <View style={styles.selectedBayBadge}>
                  <Text style={styles.selectedBayBadgeText}>🔧 Bay: {slotModalBay?.bayName || slotModalBay?.BayName || 'All'}</Text>
                </View>
                ...
                {isAllSlotsLoading ? (
                  ...
                  <View style={{ padding: 30, alignItems: 'center' }}>
                    <Text style={{ color: '#999' }}>No free slots found for today.</Text>
                  </View>
                ) : (
                  <FlatList
                    data={allSlotsModal}
                    ...
                    style={{ maxHeight: 320 }}
                    renderItem={({ item }) => {
                      const isVip = checkIsVip(item.start, item.end);
                      return (
                        <TouchableOpacity style={[styles.slotItem, isVip && { borderColor: '#FFD700', borderWidth: 1, backgroundColor: '#FFFDF0' }]} onPress={() => proceedWithSlot(item)}>
                          <Text style={styles.slotTime}>
                            {formatTimeFrom24(item.start)}
                          </Text>
                          {isVip && <Text style={{ color: '#B8860B', fontWeight: 'bold', fontSize: 12, marginBottom: 2 }}>⭐ VIP Slot</Text>}
                          <Text style={styles.slotTap}>
                            {(item.bays || item.Bays || []).length} Bay(s) Free
                          </Text>
                          {bookingData?.selectedVehicles?.length > 1 && (
                            <Text style={{
                              fontSize: 9, fontWeight: '700',
                              color: (item.bays || item.Bays || []).length >= bookingData.selectedVehicles.length ? '#1976D2' : '#E65100'
                            }}>
                              {(item.bays || item.Bays || []).length >= bookingData.selectedVehicles.length
                                ? "Parallel"
                                : "Sequential"}
                            </Text>
                          )}
...
const styles = StyleSheet.create({
  ...
  blackRatingBadge: { backgroundColor: '#1A1A1A', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, marginBottom: 6 },
...
export default viewnearbystations;